...
class Payment:
    ...
    def sum_payment(self, start_time, end_time, values_dict: dict) -> float:
        for values_dict_item in values_dict:
            sum_payment_value = 0
            start_time_values = datetime.strptime(
                values_dict_item["start"], '%H:%M')
            end_time_values = datetime.strptime(
                values_dict_item["end"], '%H:%M')
            ...
            if start_time >= start_time_values and end_time <= end_time_values:
                ...
                payment = (int(hours_worked.seconds)/3600) * \
                    (int(values_dict_item["value"]))
                sum_payment_value += payment
                logging.debug(
                    f"Start: {start_time}. End: {end_time}. Hours worked: {hours_worked}. Hour Value: {values_dict_item['value']}. Payment: {payment}. Payment Total: {sum_payment_value + self.employee_payment_total}")
                break
    ...
    def calculate_payment(self, working_hours_values_monday_friday_dict: dict, working_hours_values_saturday_sunday_dict: dict) -> dict:
        ret = {}
        for employee in self.employee_worked_hours_dict:
            ...
            for day in employee_days_dict:
                logging.debug(
                    f"Calculating values from employee {employee} from day {day}.")
                ...
                if day in self.days_monday_friday:
                    self.employee_payment_total += self.sum_payment(
                        start_time, end_time, working_hours_values_monday_friday_dict)
                else:
                    self.employee_payment_total += self.sum_payment(
                        start_time, end_time, working_hours_values_saturday_sunday_dict)
